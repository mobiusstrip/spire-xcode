...
func _ready():
	set_viewport_to_player_world()
	SoundManager.play_tracked_music("level_select_1")
	currency_editor()
...
func set_viewport_to_player_world():
	var world_index = (GameDataManager.current_world-1)*2532
	position.y = world_index
	position.x=0
	
var min_y =-0
var max_y = GameDataManager.current_world*2532 #controls upward limit *2532 for each world unlocked
func _input(event):
	if event is InputEventScreenDrag:
		var new_y = position.y - event.relative.y
		position.y = clamp(new_y,min_y,2532*17)
...
func lives():
	$CanvasLayer/ui/lives_store_button/lives_label.text=str(GameDataManager.level_info[0]["lives"])
	if GameDataManager.level_info[0]["lives"] is String:
		$CanvasLayer/ui/lives_store_button.texture_normal=full
		$CanvasLayer/ui/lives_store_button.texture_pressed=full_pressed
	else:
		if GameDataManager.level_info[0]["lives"]==5:
			$CanvasLayer/ui/lives_store_button.texture_normal=full
			$CanvasLayer/ui/lives_store_button.texture_pressed=full_pressed
		if GameDataManager.level_info[0]["lives"]==0:
			$CanvasLayer/ui/lives_store_button.texture_normal=empty
			$CanvasLayer/ui/lives_store_button.texture_pressed=empty_pressed
		else:
			$CanvasLayer/ui/lives_store_button.texture_normal=semi
			$CanvasLayer/ui/lives_store_button.texture_pressed=semi_pressed
...
func button_press(level):
	...
	if GameDataManager.level_info[0]["lives"] is int:
		...
		SoundManager.play_fixed_sound("portal")
		await get_tree().create_timer(1).timeout
		match GameDataManager.current_world:
			1:pass
			2:level+=20
			3:level+=20
		Transition.change_scene_to_file("res://scenes/level_scenes/"+str(level)+".tscn","fade_purple")
...
func _on_1_pressed():
	if $"levels/1".enabled==true and wait==false:
		SoundManager.play_fixed_sound("button_pressed")
		if $confirmation.visible==false:
			$confirmation.visible=true
		else:
			button_press(1)
			GameDataManager.modify_dict(0,"character_visibility","right")
...
func _on_7_pressed():
	if $"levels/7".enabled==true and wait==false:
		...
		if $confirmation.visible==false:
			...
			button_press(7)
			GameDataManager.modify_dict(0,"character_visibility","left")
...
func _on_pause_button_pressed():
	SoundManager.play_fixed_sound("button_pressed")
	if $CanvasLayer/ui.visible==true:
...
func _on_lives_store_6h_buy_button_pressed():
	if GameDataManager.level_info[0]["shards"]>=int($lives_store/lives_store_6h_buy_button/lives_price.text):
		...
		bought_booster_fx()
		$lives_store.visible=false
		$CanvasLayer/ui/lives_store_button/lives_label.text=GameDataManager.level_info[0]["lives"]
	else:
